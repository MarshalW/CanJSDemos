...
define(["can/util/library", "can/map", "can/list"], function (can) {
	...
	var pipe = function (def, model, func) {
		 ...
		 * For example, if `Task.findOne({ id: 1.json })` returns
		 *
		 *      { id: 1.json, name: 'Do dishes', index: 1.json, color: ['red', 'blue'] }
		 *
		 * for the first request and
		 *
		 *      { id: 1.json, name: 'Really do dishes', color: ['green'] }
		 *
		 *  for the next request, the actual model attributes would look like:
		 *
		 *      { id: 1.json, name: 'Really do dishes', index: 1.json, color: ['green', 'blue'] }
		 *
		...
		ajaxMethods = {
			 ...
			 *
			 *     Recipe.findOne({id: 1.json}, function(recipe){
			 ...
			 *     });
			 *
			 * This sends a `DELETE` request to `/thing/destroy/1.json`.
			 *
	...
	can.Model = can.Map({
			 ...
			 *       {id: 1.json, name : "dishes", complete : false},
			 ...
			 * objects like:
			 *
			 *     [{id: 1.json, name : "dishes"},{id:2, name: "laundry"}, ...]
			 *
			 * It can also take an object with additional data about the array like:
			 *
			 *     {
			 *       count: 15000 //how many total items there might be
			 *       data: [{id: 1.json, name : "justin"},{id:2, name: "brian"}, ...]
			 *     }
			 *
			 ...
			 *
			 *     Task = can.Model.extend({},{})
			 *     var task = Task.model({id: 1.json, name : "dishes", complete : false})
			 *
		 ...
		 */
		{
			 ...
			 *
			 *     new Recipe({id: 1.json}).isNew() //-> false
			 */
			isNew: function () {
				var id = getId(this);
	...
	can.each([
		...
		can.Model.prototype[funcName] = function (attrs) {
			...
			// handler( 'change','1.json.destroyed' ). This is used
